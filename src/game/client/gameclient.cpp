...
void CGameClient::OnInit()
{
	...
	const char *pLoadingDDNetCaption = Localize("Loading chillerbot-ux");
...
int CGameClient::OnSnapInput(int *pData, bool Dummy, bool Force)
{
	...
	{
		mem_copy(pData, &m_VibeBot.m_aInputData[CurrentTee], sizeof(m_VibeBot.m_aInputData[CurrentTee]));
		return sizeof(m_VibeBot.m_aInputData[CurrentTee]);
	}
	if(m_TerminalUI.m_SendData[CurrentTee])
	{
		mem_copy(pData, &m_TerminalUI.m_aInputData[CurrentTee], sizeof(m_TerminalUI.m_aInputData[CurrentTee]));
		return sizeof(m_TerminalUI.m_aInputData[CurrentTee]);
	}
	if(m_Stresser.m_SendData[CurrentTee])
	{
		mem_copy(pData, &m_Stresser.m_aInputData[CurrentTee], sizeof(m_Stresser.m_aInputData[CurrentTee]));
		return sizeof(m_Stresser.m_aInputData[CurrentTee]);
	}
	if(!Dummy)
	{
		...
		else if(Item.m_Type == NETEVENTTYPE_SOUNDWORLD)
		{
			...
			if(!g_Config.m_SndLongPain)
				if(pEvent->m_SoundId == SOUND_PLAYER_PAIN_LONG)
					SoundId = SOUND_PICKUP_HEALTH;

			m_Sounds.PlayAt(CSounds::CHN_WORLD, SoundId, 1.0f, vec2(pEvent->m_X, pEvent->m_Y));
		}
	}
}
